...
const navItems = [
    ...
    { name: 'My Requests', href: '/portal/requests', icon: FileText, badge: null },
    { name: 'Messages', href: '/portal/chat', icon: MessageSquare, badge: '2' },
    { name: 'Receipts', href: '/portal/receipts', icon: Receipt },
    { name: 'Settings', href: '/portal/settings', icon: Settings },
];

export default function PortalSidebar({ isOpen, onClose, collapsed, setCollapsed }) {
                    ...
                    {!collapsed ? (
                        <Link to="/portal/dashboard" className="flex items-center space-x-3 group">
                            <img
                                src="logo.png"
                                alt="Optivis"
                                className="w-30 h-30 object-contain drop-shadow-lg group-hover:scale-110 transition-transform"
                            />
                            <div className="overflow-hidden">
                                <h2 className="text-lg font-serif font-bold tracking-tight text-white flex items-center gap-1.5">
                                    Optivis <span className="text-gold text-xs px-1.5 py-0.5 rounded bg-gold/20 font-sans uppercase tracking-wider font-semibold">Portal</span>
                                </h2>
                                <p className="text-[11px] text-gray-300 font-sans tracking-wide truncate">Client Workspace</p>
                            </div>
                        </Link>
                    ) : (
                        <Link to="/portal/dashboard" className="mx-auto group" title="Optivis Client Portal">
                    ...
                    <div className={`flex items-center ${collapsed ? 'justify-center' : 'space-x-3 bg-white/5 p-2 rounded-xl border border-white/5'}`}>
                        ...
                        {!collapsed && (
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-white truncate">{user?.fullName || 'Valued Client'}</p>
                                <p className="text-xs text-gray-300 truncate">{user?.company || user?.email || 'Verified Account'}</p>
                            </div>
                        )}
                    </div>
                </div>
